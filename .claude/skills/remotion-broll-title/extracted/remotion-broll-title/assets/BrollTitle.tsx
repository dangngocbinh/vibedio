...
export const BrollTitle: React.FC<BrollTitleProps> = ({
  ...
  const containerStyle: React.CSSProperties = {
    ...
    transform: positionStyle.transform 
      ? `${positionStyle.transform} ${transform}`.trim()
      : transform || undefined,
  };
  ...
  const contentStyle: React.CSSProperties = {
    backgroundColor,
    padding: style === 'corner-badge' ? '12px 20px' : '16px 28px',
    borderLeft: showAccentLine && style !== 'centered' ? `4px solid ${accentColor}` : undefined,
    borderRadius: style === 'corner-badge' ? '4px' : undefined,
  };
      ...
      <div style={containerStyle}>
        <div style={contentStyle}>
          <h1
            style={{
              color: textColor,
              fontSize: style === 'full-screen' ? fontSize * 1.5 : fontSize,
              fontWeight: 700,
              margin: 0,
